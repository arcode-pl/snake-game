...
static int clientSocketList[DEFAULT_MAX_CLIENTS];
...
int SocketUtil_createServer(SocketUtil_t *su) 
{
	...
	memset(clientSocketList, 0, DEFAULT_MAX_CLIENTS * sizeof(int));
...
int SocketUtil_initialize(SocketUtil_t *su) 
{
	...
	if (su->config.maxClients > 0) // run as server
	{
		res = SocketUtil_createServer(su);
	}
	else // run as client
	{
		res = SocketUtil_connectToServer(su);
	}
